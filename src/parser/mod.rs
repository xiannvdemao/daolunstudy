...
mod tests {
    ...
    fn test_parse_select_with_where() {
        ...
        match result.unwrap() {
            Statement::Select(s) => {
                ...
                if let Expression::BinaryOp(left, op, right) = &s.where_clause.unwrap() {
                    assert_eq!(op, "=");
    ...
    #[test]
    fn test_parse_select_with_string_condition() {
        let result = parse("SELECT * FROM users WHERE name = 'Alice'");
        assert!(result.is_ok());
        match result.unwrap() {
            Statement::Select(s) => {
    ...
    #[test]
    fn test_parse_select_with_comparison_operators() {
        let result = parse("SELECT * FROM users WHERE age > 18");
        assert!(result.is_ok());
        let result2 = parse("SELECT * FROM users WHERE age < 30");
        assert!(result2.is_ok());
        let result3 = parse("SELECT * FROM users WHERE age >= 18");
        assert!(result3.is_ok());
        let result4 = parse("SELECT * FROM users WHERE age <= 30");
        assert!(result4.is_ok());
        let result5 = parse("SELECT * FROM users WHERE age != 25");
    ...
    fn test_parse_insert_null_value() {
        let result = parse("INSERT INTO users (id, name) VALUES (1, NULL)");
        assert!(result.is_ok());
        match result.unwrap() {
            Statement::Insert(i) => {
                assert_eq!(i.table, "users");
                assert_eq!(i.values[0][1], Expression::Literal("NULL".to_string()));
            }
            _ => panic!("Expected INSERT statement"),
        }
    }
    ...
    fn test_parse_insert_negative_number() {
        let result = parse("INSERT INTO items VALUES (-100, 'Debt')");
        assert!(result.is_ok());
        match result.unwrap() {
            Statement::Insert(i) => {
                assert_eq!(i.values[0][0], Expression::Literal("-100".to_string()));
            }
            _ => panic!("Expected INSERT statement"),
        }
    }
    ...
    #[test]
    fn test_parse_update_null_value() {
        let result = parse("UPDATE users SET email = NULL WHERE id = 1");
        assert!(result.is_ok());
        match result.unwrap() {
            Statement::Update(u) => {
                assert_eq!(u.set_clauses[0].1, Expression::Literal("NULL".to_string()));
            }
            _ => panic!("Expected UPDATE statement"),
        }
    }
    ...
    #[test]
    fn test_parse_delete_with_comparison() {
        let result = parse("DELETE FROM users WHERE age > 100");
        assert!(result.is_ok());
        match result.unwrap() {
            Statement::Delete(d) => {
                assert!(d.where_clause.is_some());
    ...
    #[test]
    fn test_parse_insert_missing_parentheses() {
        let result = parse("INSERT INTO users VALUES 1, 2, 3");
        assert!(result.is_err());
    }

    #[test]
    fn test_parse_update_missing_set() {
        let result = parse("UPDATE users name = 'Bob'");
        assert!(result.is_err());
    }
    ...
    fn test_parse_empty_statement() {
        ...
        assert!(err.contains("Eof") || err.contains("Empty"));
    }

    #[test]
    fn test_parse_unknown_statement() {
        let result = parse("UNKNOWN STATEMENT");
        assert!(result.is_err());
    }

    #[test]
    fn test_parse_unclosed_parentheses() {
        let result = parse("SELECT * FROM (users WHERE id = 1");
        assert!(result.is_err());
    }
    ...
    fn test_parse_invalid_expression() {
        let result = parse("SELECT * FROM users WHERE = 1");
        assert!(result.is_err());
    }

    #[test]
    fn test_parse_missing_value_in_set() {
        let result = parse("UPDATE users SET name = ");
        assert!(result.is_err());
    }
}
